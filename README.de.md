...
|------|-----------|
| `0` | Erfolg. |
| `1` | Assertion-Fehler - eine oder mehrere Assertions sind fehlgeschlagen, oder ein `batch`-Lauf hat nichts geprüft. Nur `batch` liefert ein Assertion-Urteil; `compare` kann bei einem unerwarteten Fehler dennoch mit `1` enden. |
...
**Datenschutzhinweis:** Jedes Ausgabeformat - JSON, CSV und Markdown - enthält den vollständigen Prompt und die vollständige Modellantwort jedes Ergebnisses sowie eventuelle Provider-Fehlermeldungen. JSON enthält zusätzlich den Begründungstext jedes Judge-Modells; `--include-reasoning` steuert nur die Konsolenanzeige, nicht die Datei, und CSV und Markdown enthalten ihn nicht. Behandeln Sie jede Ausgabedatei als sensibel, bevor Sie sie committen oder als öffentliches CI-Artefakt hochladen. Aufbewahrungs- und Trainingsbedingungen unterscheiden sich je nach Anbieter, dieses Tool trifft dazu keine Aussage, und Sie sollten die Bedingungen jedes von Ihnen konfigurierten Anbieters prüfen. Claude Fable 5.1 erfordert eine 30-tägige Aufbewahrung und ist nicht unter Zero Data Retention verfügbar.
...
| Moonshot AI / Kimi (4 registrierte IDs: K3, K2.7 Code, K2.7 Code HighSpeed, K2.6) | ✅ | ✅ | ✅ |
